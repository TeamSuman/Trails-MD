...
| 3.2 | Critical | Delta checkpointing makes each `history.pkl` non-self-contained; deleting/losing one dir silently drops that slice of history forever (breaks lineage/MSM). `manager.py`. | **[PARTIAL]** each checkpoint now records its delta chain (`history_chain.json`) and `reconstruct_history` **loudly reports** a broken chain instead of silent loss (+test); making history fully self-contained remains **[OPEN]**. |
...
| 3.4 | High | Resume is non-deterministic: numpy/torch/Python RNG state is never checkpointed, so a resumed run diverges from an uninterrupted one despite the "exact deterministic restart" claim. | **[FIXED]** RNG state (Python/NumPy/torch, torch stored as NumPy so the checkpoint stays torch-independent) captured in `sampler_state` and restored on resume + test. |
