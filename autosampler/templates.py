"""Canonical, fully-annotated AutoSampler input file.

``DEFAULT_TEMPLATE`` is the single source of truth for the starter config emitted
by ``autosampler-init`` and mirrored to ``examples/template.yaml``. It documents
every section, the available method choices, and sensible defaults.
"""
...
# this file. Validate before running:  autosampler --config config.yaml --check
